...
export const authenticateUserFunction = (context) => {
    const authenticateUser = async (authId, identifier, password) => {
        ...
        if (account.hashed_password) {
            if (account.hashed_password.startsWith("$2a")) {
                console.log(`${chalk.red.bold("[LUCIA_ERROR]")} ${chalk.red("The hashing algorithm used for passwords has changed as of v0.8.0, and all accounts made pre-v0.8.0 that uses passwords are invalid.")}`);
                process.exit(1);
            }
            const isValid = await verify(password || "", account.hashed_password);
            if (!isValid)
