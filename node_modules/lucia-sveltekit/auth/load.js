...
import { FingerprintToken, EncryptedRefreshToken, AccessToken, } from "../utils/token.js";
import { createAccessToken, createRefreshToken, getAccountFromDatabaseData, } from "../utils/auth.js";
export const handleServerSessionFunction = (context) => {
    const handleServerSessionCore = async ({ cookies, }) => {
        const fingerprintToken = new FingerprintToken(cookies.get("fingerprint_token") || "", context);
        const encryptedRefreshToken = new EncryptedRefreshToken(cookies.get("encrypt_refresh_token") || "", context);
        try {
            const accessToken = new AccessToken(cookies.get("access_token") || "", context);
            ...
            const newRefreshToken = await createRefreshToken(userId, fingerprintToken.value, context);
            ...
            const newAccessToken = await createAccessToken(account.user, fingerprintToken.value, context);
            const result = {
                user: account.user,
                access_token: newAccessToken.value,
                refresh_token: newRefreshToken.value,
            };
            setCookie(cookies, newAccessToken.cookie(), newEncryptedRefreshToken.cookie());
            return {
                _lucia: result,
            };
        }
