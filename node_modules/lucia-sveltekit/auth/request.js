...
import { AccessToken, FingerprintToken, EncryptedRefreshToken, } from "../utils/token.js";
import { Error } from "../index.js";
export const validateRequestFunction = (context) => {
    const validateRequest = async (request) => {
        const clonedReq = request.clone();
        const authorizationHeader = clonedReq.headers.get("Authorization") || "";
        ...
        const fingerprintToken = new FingerprintToken(cookies.fingerprint_token, context);
        const accessToken = new AccessToken(token, context);
        const encryptedRefreshToken = new EncryptedRefreshToken(cookies.encrypt_refresh_token, context);
...
export const validateRequestByCookieFunction = (context) => {
    const validateRequestByCookie = async (request) => {
        const clonedReq = request.clone();
        const method = clonedReq.method;
        if (method !== "GET")
            throw new Error("AUTH_INVALID_REQUEST_METHOD");
        const cookies = cookie.parse(clonedReq.headers.get("cookie") || "");
        const fingerprintToken = new FingerprintToken(cookies.fingerprint_token, context);
